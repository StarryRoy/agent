...
class ProcurementServices:
    ...
    def parse_requirement(self, task: str) -> dict[str, Any]:
        ...
        if late_match and request.get("latest_delivery_date"):
            ...
            shifted = date.fromisoformat(str(request["latest_delivery_date"])) + timedelta(days=delta)
    ...
    def analyze_inventory(self, task: str) -> dict[str, Any]:
        ...
        if request.get("simulate_subagent_failure"):
        ...
        sql = """
            ...
            GROUP BY p.id, p.sku, p.name, i.current_qty, i.locked_qty,
                     i.in_transit_qty, i.safety_stock, i.updated_at
        """
        if request.get("simulate_sql_failure"):
            sql = "SELECT * FROM missing_inventory_table WHERE product_id = :product_id"
        result, query = self._query("inventory_and_consumption", sql, {"product_id": request.get("product_id")})
    ...
    def analyze_suppliers(self, task: str) -> dict[str, Any]:
        ...
        if request.get("simulate_subagent_failure") == "supplier":
        ...
        """
        result, query = self._query("supplier_capability_and_history", sql, {"product_id": request.get("product_id")})
    ...
    def analyze_pricing(self, task: str) -> dict[str, Any]:
        ...
        """
        result, query = self._query("current_and_historical_prices", sql, {"product_id": request.get("product_id")})
        ...
        for row in _rows(result):
            ...
            if variance is not None and abs(variance) > 0.15:
                anomalies.append({"supplier": row["code"], "type": "price_variance", "rate": variance})
        ...
        eligible = [
            ...
            and (days is None or int(quote_by_code[item["code"]]["lead_time_days"]) <= int(days))
        ...
        def make_plan(name: str, allocations: list[dict[str, Any]]) -> dict[str, Any]:
            ...
            total = round(sum(float(item["unit_price"]) * int(item["quantity"]) for item in allocations), 2)
                        ...
                        [
                            {
                                ...
                                "lead_time_days": int(quote["lead_time_days"]),
                            }
                        ],
                    )
                )
        ...
        remaining = qty
        for candidate in sorted(eligible, key=lambda item: float(quote_by_code[item["code"]]["unit_price"])):
            if remaining <= 0:
                break
            quote = quote_by_code[candidate["code"]]
            capacity = min(int(candidate["max_capacity"]), int(quote["available_qty"]))
        ...
        plans = sorted(unique.values(), key=lambda item: (not item["meets_quantity"], item["total_cost"]))
    ...
    def analyze_budget(self, task: str) -> dict[str, Any]:
        ...
        query["facts"] = [
            ...
            f"用户预算上限{float(user_budget):.2f}" if user_budget is not None else "用户未设置单独上限",
    ...
    def analyze_risk(self, task: str) -> dict[str, Any]:
        ...
        """
        result, query = self._query("supplier_risk_records", sql, {"product_id": request.get("product_id")})
        ...
        for plan in pricing.get("plans") or []:
            ...
            for allocation in allocations:
                ...
                on_time_rate = float(record.get("on_time_deliveries") or 0) / deliveries if deliveries else 0
            ...
            if effective_budget and float(plan["total_cost"]) > effective_budget:
                ...
                items.append("超出有效预算")
            score = min(score, 100)
            level = "low" if score <= 20 else "medium" if score <= 40 else "high" if score <= 70 else "critical"
        ...
        return {
            ...
            "replan_reason": None if recommended else self._replan_reason(assessed, effective_budget),
            "facts": query["facts"],
            "conclusion": query["conclusion"],
    ...
    def _replan_reason(plans: list[dict[str, Any]], effective_budget: float) -> str:
        if plans and effective_budget and all(float(item["total_cost"]) > effective_budget for item in plans):
    ...
    def execute_plan(self, plan_json: str) -> dict[str, Any]:
        ...
        if request.get("simulate_execution_failure"):
            return {
                "status": "failed",
                "error": {"type": "scripted_execution_failure", "message": "执行前外部采购系统失败"},
                "executed_actions": [],
            }
        ...
        budget_check = self.database.execute_query(
            """
            SELECT b.id, b.total_amount - b.used_amount - b.approved_pending_amount AS available_amount
            FROM budgets b JOIN departments d ON d.id = b.department_id
            WHERE d.code = :code AND b.fiscal_year = :year
            """,
            {"code": request.get("department_code", "IT"), "year": self.today.year},
        )
        budget_rows = _rows(budget_check)
        total_cost = float(plan["total_cost"])
        if not budget_rows or float(budget_rows[0]["available_amount"]) < total_cost:
            return {
                "status": "failed",
                "error": {"type": "budget_changed", "message": "执行时可用预算不足"},
                "executed_actions": [],
            }
        ...
        request_write = self.database.execute_write(
            ...
            INSERT INTO purchase_requests(
                request_no, department_id, product_id, requested_qty, approved_qty,
            ...
            """,
            {
                ...
                "department_id": envelope.get("budget_analysis", {}).get("department", {}).get("id", 1),
        ...
        if not request_write.get("ok"):
            return {"status": "failed", "error": request_write.get("error"), "executed_actions": actions}
        request_id = request_write["last_insert_id"]
        actions.append({"action": "create_purchase_request", "id": request_id, "request_no": request_no})

        for index, allocation in enumerate(plan["allocations"], start=1):
            order_no = f"PO-{self.today:%Y%m%d}-{nonce}-{index}"
            expected = self.today + timedelta(days=int(allocation["lead_time_days"]))
            order_write = self.database.execute_write(
                """
                INSERT INTO purchase_orders(
                    order_no, request_id, supplier_id, quantity, unit_price, total_amount,
                    expected_delivery_date, status, created_at
                ) VALUES(
                    :order_no, :request_id, :supplier_id, :quantity, :unit_price,
                    :total_amount, :expected_date, 'created', :created_at
                )
                """,
                {
                    "order_no": order_no,
                    ...
                    "total_amount": float(allocation["unit_price"]) * int(allocation["quantity"]),
                    "expected_date": expected.isoformat(),
                    "created_at": now,
                },
            )
            if not order_write.get("ok"):
                self._mark_execution_failed(request_id, session_id, order_write, now)
                return {"status": "failed", "error": order_write.get("error"), "executed_actions": actions}
            actions.append({"action": "create_purchase_order", "id": order_write["last_insert_id"], "order_no": order_no})

        budget_write = self.database.execute_write(
            """
            UPDATE budgets SET approved_pending_amount = approved_pending_amount + :amount
            WHERE id = :budget_id
              AND total_amount - used_amount - approved_pending_amount >= :amount
            """,
            {"amount": total_cost, "budget_id": budget_rows[0]["id"]},
        )
        if not budget_write.get("ok") or int(budget_write.get("rows_affected", 0)) != 1:
            self._mark_execution_failed(request_id, session_id, budget_write, now)
            return {
                "status": "failed",
                "error": budget_write.get("error") or {"type": "budget_race", "message": "预算占用失败"},
                "executed_actions": actions,
            }
        actions.append({"action": "reserve_budget", "amount": total_cost})

        status_write = self.database.execute_write(
            "UPDATE purchase_requests SET status = 'ordered' WHERE id = :id",
            {"id": request_id},
        )
        actions.append({"action": "update_purchase_status", "result": status_write})
        approval_log = self.database.execute_write(
            """
            INSERT INTO operation_logs(session_id, entity_type, entity_id, action, result, details_json, created_at)
            VALUES(:session_id, 'purchase_request', :entity_id, 'record_approval', 'approved', :details, :created_at)
            """,
            {
                "session_id": session_id,
                "entity_id": request_id,
                "details": _json({"request_no": request_no, "total_amount": total_cost}),
                "created_at": now,
            },
        )
        actions.append({"action": "write_approval_and_operation_log", "result": approval_log})
        return {
            "status": "success",
    ...
    def _mark_execution_failed(
        self, request_id: int, session_id: str, failure: dict[str, Any], created_at: str
    ) -> None:
        self.database.execute_write(
            "UPDATE purchase_requests SET status = 'execution_failed' WHERE id = :id",
            {"id": request_id},
        )
        self.database.execute_write(
            """
            INSERT INTO operation_logs(session_id, entity_type, entity_id, action, result, details_json, created_at)
            VALUES(:session_id, 'purchase_request', :entity_id, 'execution', 'failed', :details, :created_at)
            """,
            {
                "session_id": session_id,
                "entity_id": request_id,
                "details": _json(failure),
    ...
    def tools(self) -> dict[str, BaseTool]:
        ...
        definitions = {
            "parse_requirement": (self.parse_requirement, "Parse and merge a procurement requirement."),
            "analyze_inventory": (self.analyze_inventory, "Analyze inventory, forecast use, and purchase gap."),
            "analyze_suppliers": (self.analyze_suppliers, "Screen suppliers using database evidence."),
            "analyze_pricing": (self.analyze_pricing, "Compare quotations and build cost combinations."),
            "analyze_budget": (self.analyze_budget, "Validate the proposal against department and user budgets."),
            "analyze_risk": (self.analyze_risk, "Assess supplier, delivery, quality, price, and concentration risk."),
            "execute_procurement_plan": (self.execute_plan, "Execute one approved procurement plan."),
        }
