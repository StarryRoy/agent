...
def build_mock_execute_query_tool(role: str) -> BaseTool:
    """Build the explicit deterministic-test query Tool backed by JSON rows."""
...
class DeterministicProcurementModel(BaseChatModel):
    ...
    def _subagent(self, messages: list[BaseMessage]) -> AIMessage:
        ...
        if (
            self.role == "supplier"
            and primary_messages
            and not external_messages
            and "supplier_status" in self.bound_tool_names
        ):
            analysis = _tool_value(primary_messages[-1])
            candidates = (
        ...
        if self.role == "supplier" and primary_messages and external_messages:
            analysis = _tool_value(primary_messages[-1])
            external = _tool_value(external_messages[-1])
